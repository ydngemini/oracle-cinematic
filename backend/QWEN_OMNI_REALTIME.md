...
`DASHSCOPE_API_KEY`, `DASHSCOPE_WORKSPACE_ID`, or the existing
`ORACLE_ACS_WEBHOOK_SECRET` makes production startup fail closed.
...
Do not log or expose the DashScope API key. The ACS media endpoint authenticates
with a constant-time comparison against `ORACLE_ACS_WEBHOOK_SECRET`.
