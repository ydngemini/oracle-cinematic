...
| `data_integrations/listings_feed.py` | RESO Web API (licensed) | **IDX (assumed) / Licensed API (actual)** | `ORACLE_RESO_URL`, `ORACLE_RESO_TOKEN`, `ORACLE_RESO_MLS_ID` OR `ORACLE_RESO_FEEDS_JSON` | Hourly delta sync of MLS listings via OData. Multiple boards via JSON config. Upserts into `oracle_mls_listings` through shared `mls_sink.py`. **Before use: confirm which tier the feed agreement grants — analytics/enrichment over IDX feeds is a classic breach.** |
| `data_integrations/bridge_listings_feed.py` | Bridge Data Output API | **Licensed API** | `ORACLE_BRIDGE_ENABLED`, `ORACLE_BRIDGE_DATASET`, `ORACLE_BRIDGE_ACCESS_TOKEN`, `ORACLE_BRIDGE_MLS_ID` | Bridge's own JSON REST protocol (NOT OData). Separate module from RESO due to incompatible query syntax (`$filter`/`$top` rejected). Test datasets tag provenance as `developer_listing_dataset`; licensed boards tagged as `licensed_property_listing`. Upserts through same `mls_sink.py`. Multiple boards require separate Bridge approvals even with same platform token. |
